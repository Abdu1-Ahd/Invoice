...
- **`Invoice`**: `id`, `customerId`, `invoiceNumber`, `status` (`Draft`|`Sent`|`Paid`|`Overdue`), `issueDate`, `dueDate`, `billingCycle`, `currency`, `discount`, `subtotal`, `taxRate`, `taxAmount`, `totalAmount`, `notes`, `terms`, `createdAt`, `updatedAt`, `deletedAt`
...
- **`SyncQueue`**: `id`, `entityType`, `entityId`, `operation`, `payload`, `status`, `createdAt`
